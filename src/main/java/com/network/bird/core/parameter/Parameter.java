package com.network.bird.core.parameter;

import com.network.bird.common.enums.ErrorCode;
import com.network.bird.common.exception.BaseException;
import com.network.bird.core.parameter.criterion.Criterion;
import com.network.bird.core.parameter.criterion.CriterionAnd;
import com.network.bird.core.parameter.criterion.CriterionOr;
import com.network.bird.core.parameter.criterion.Query;
...
public abstract class Parameter implements Serializable {
    ...
    private Logger log = LoggerFactory.getLogger(Parameter.class);
    ...
    public Parameter addIncludeField(String... fields) {
        ...
        for (String field : fields) {
            if (field != null && field.matches("[0-9a-zA-Z_-]+")) {
                ...
                throw new BaseException("参数错误:" + field);
            }
        }
        return this;
    }
     ...
     * @param query 搜索条件
     * @return Parameter的实例
     * @see com.network.bird.core.parameter.criterion.Query
     */
    ...
    public Parameter addSort(String sortField, boolean isAsc) {
        if (sortField != null && sortField.matches("[0-9a-zA-Z_-]+")) {
            ...
            throw new BaseException(ErrorCode.PARAMETER_ERROR, "参数错误:" + sortField);
        }
        return this;
    }
    ...
    public String getQuery(Map<String, String> mapping) {
        if (!validate()) {
            throw new BaseException("查询条件存在错误!");
        }
        List<Criterion> cList = getAllCriterion();
        if (cList.isEmpty()) {
            return "";
        }
        //将查询条件转换为字符串
        ...
        if (mapping == null || mapping.isEmpty()) {
            ...
            for (int i = 0; i < propertiesList.size(); i++) {
                ...
                if (param != null) {
                    ...
                    throw new BaseException("没有找到属性" + propertiesList.get(i) + "的映射关系");
                }
            }
        }
    ...
    public List<Object> getQueryValue() {
        ...
        List<Criterion> list = getAllCriterion();
    ...
    private List<Criterion> getAllCriterion() {
        List<Criterion> cList = new LinkedList<>();
        //将类中非null的属性添加到查询条件中
        ...
        List<String> propertiesList = this.getPropertiesByCriterions(cList);
        List<Criterion> defaultValues = getDefaultValue();
        //添加默认查询条件
        for (Criterion c : defaultValues) {
            if (c instanceof CriterionAnd || c instanceof CriterionOr) { //直接添加
                cList.add(c);
            } else if (!propertiesList.contains(c.getPropertyName().get(0))) { //如果已经设置查询条件，则不添加默认的查询条件
                cList.add(c);
            }
        }
        return cList;
    ...
    public void setFirstResult(int firstResult) {
        if (firstResult < 0) {
            throw new BaseException(ErrorCode.PARAMETER_ERROR, "参数不能小于0");
        } else {
    ...
    public void setMaxResults(int maxResults) {
        if (maxResults < 0 || maxResults > 1000) {
            throw new BaseException(ErrorCode.PARAMETER_ERROR, "参数应大于0,并且小于1001!");
        } else {
    ...
    public boolean validate() {
        ...
        List<String> includes = getIncludeField();
        flag = includes.stream().allMatch(name::contains);
        return flag;
    }
